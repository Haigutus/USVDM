...
def dic_to_xml(conf_dic):
    """Create XML file from dictionary conf input"""
    end_key       = len(conf_dic["XML_MESSAGE"])-1
    current_key   = 0
    ...
    while current_key <= end_key:
        ...
        element_name = conf_dic["XML_MESSAGE"][str(current_key)]["DATA"]["element"]
        ...
        if (current_key == 0):
            ...
            parent_name  = xml_elements_dic[conf_dic["XML_MESSAGE"][str(current_key)]["PARRENT"]]

            element = etree.SubElement(parent_name, element_name)

            #Set Element attributes

        if "attributes" in conf_dic["XML_MESSAGE"][str(current_key)]["DATA"]:

            for attrib_key in conf_dic["XML_MESSAGE"][str(current_key)]["DATA"]["attributes"]:

                element.attrib[attrib_key] = conf_dic["XML_MESSAGE"][str(current_key)]["DATA"]["attributes"][attrib_key]

        #Set Element text value

        element.text = conf_dic["XML_MESSAGE"][str(current_key)]["DATA"].get("text", "")
